...
try:
    buys  = []
    sales = []
    month = int(input('Input number of needed month (Except current one) :'))
    year = int(input('Input needed year:'))
    choice = int(input('Do you wanna input restricted amount of days manually? (1/0)'))
    if not choice: amount = getAmount(month)
    else: amount = int(input('Input amount of days you need:'))
    currency = input('Input needed currency (CHF/RUB/GBP/USD/EUR/PLZ):')
